...
def env(tmp_path):
    ...
    e.camera, e.sink, e.session = Camera(e.clock), Sink(), SESSION
    e.make = lambda **kw: CommandHandler(
        device_id="snn-pi", state=e.state, camera=kw.get("camera", e.camera), sink=kw.get("sink", e.sink),
        session=lambda: e.session, enqueue_ack=e.acks.append, utc_now=e.clock.utc, monotonic=e.clock.monotonic,
    )  # fmt: skip
    e.handler = e.make()
    return e
...
def test_a_command_that_is_not_a_capture_is_refused_and_never_actuates(env):
    alarm = fixture("alarm-command") | {"device_id": "snn-pi", "session_id": "s1", "epoch": 1, "command_id": "a1"}
    assert env.handler.handle(alarm) == "failed:UNSUPPORTED_COMMAND"
