...
def compute_new_features(x_f: np.ndarray, fs: float) -> np.ndarray:
    ...
    return np.column_stack(
        [hjorth, autocorr, curve, crest, flat, centroid, band_low]
    ).astype(np.float32)
...
def _provenance(arch_dir, dataset_version, seed, warmup_seconds):
    ...
    return {
        ...
        "split": {
            "grouped_by": "miks VOICe (synthetic_NNN), wspólnie dla obu klas",
            "seed": seed,
        },
...
def _splits_from_manifest(arch_dir, version):
    ...
    if not os.path.exists(man):
        print(
            f"[!] nie znaleziono {man} — używam własnego podziału po grupach",
            flush=True,
        )
        return None
    import csv as _csv
    ...
    print(
        f"[pochodzenie] podział wzięty z manifestu {version} ({len(out)} rekordów)",
        flush=True,
    )
    return out
...
def main():
    ...
    ap.add_argument(
        "--glass-dir", default=None, help="domyślnie <arch>/voice_extracted/glass"
    )
    ap.add_argument(
        "--neg-dir", default=None, help="domyślnie <arch>/voice_extracted/hard_negative"
    )
    ...
    ap.add_argument(
        "--limit", type=int, default=None, help="max plików/klasę (smoke test)"
    )
    ap.add_argument(
        "--dataset-version",
        default="voice_extracted (niewersjonowane)",
        help="wersja zbioru audio, z której powstaje ten artefakt, np. v1.0.0",
    )
    args = ap.parse_args()

    prov = _provenance(
        args.arch_dir, args.dataset_version, args.seed, args.warmup_seconds
    )
    print(
        f"[pochodzenie] audio={prov['dataset_version']}  "
        f"encoder={str(prov['encoder_sha256'])[:12]}…  "
        f"bank={prov['feature_bank_sha256'][:12]}…",
        flush=True,
    )
    ...
    glass_dir = args.glass_dir or os.path.join(
        args.arch_dir, "voice_extracted", "glass"
    )
    neg_dir = args.neg_dir or os.path.join(
        args.arch_dir, "voice_extracted", "hard_negative"
    )
    ...
    if not glass or not neg:
        sys.exit(
            f"brak .wav (glass={len(glass)} w {glass_dir}, neg={len(neg)} w {neg_dir})"
        )
    print(
        f"[dane] glass {len(glass)} (poz.), hard_negative {len(neg)} (neg.)  "
        f"kanały={len(ALL_NAMES)}: {','.join(ALL_NAMES)}",
        flush=True,
    )
    ...
    if man_split:
        ...
        all_test = {
            f for f in glass + neg if man_split.get(os.path.abspath(f)) == "test"
        }
        brak = [f for f in glass + neg if os.path.abspath(f) not in man_split]
        if brak:
            sys.exit(
                f"[BŁĄD] {len(brak)} plików nie ma w manifeście {args.dataset_version} "
        ...
        all_val, all_test = _split_groups(
            glass + neg, args.val_frac, args.test_frac, rng
        )
    ...
    for f in glass + neg:
        _gr["test" if f in all_test else "val" if f in all_val else "train"].add(
            _group_of(f)
        )
    _leak = (
        (_gr["train"] & _gr["test"])
    ...
    print(
        f"[split] miksy: train={len(_gr['train'])} val={len(_gr['val'])} "
        f"test={len(_gr['test'])} | wspólnych={len(_leak)}",
        flush=True,
    )
    print(
        f"[split] pliki: train={len(glass) + len(neg) - len(all_val) - len(all_test)} "
    ...
    for i, (label, f) in enumerate(stream):
        ...
        if (i + 1) % 500 == 0:
            print(
                f"[enc] {i + 1}/{len(stream)} plików ({time.time() - t0:.0f}s)",
                flush=True,
            )
    ...
    tr_new_pos = (
        np.concatenate([r[3] for r in recs if r[1] == "train" and r[0] == 1])
        if any(r[1] == "train" and r[0] == 1 for r in recs)
        else np.zeros((0, len(NEW_NAMES)), np.float32)
    )
    tr_new_neg = (
        np.concatenate([r[3] for r in recs if r[1] == "train" and r[0] == 0])
        if any(r[1] == "train" and r[0] == 0 for r in recs)
        else np.zeros((0, len(NEW_NAMES)), np.float32)
    )
    if len(tr_new_neg) == 0:
    ...
    for c, name in enumerate(NEW_NAMES):
        ...
        print(
            f"  {name:20s} fire gdy v {cmp} {thr:.4f}  "
    ...
    for split, (wins, labs, fidxs, filelab) in split_data.items():
        ...
        np.savez_compressed(
            cache, win=win, lab=lab, fidx=fidx, file_lab=file_lab, sig=sig
        )
        json.dump(
            {
                "channels": ALL_NAMES,
        ...
        print(
            f"[ok] {split}: {len(lab)} okien / {len(filelab)} klipów, "
