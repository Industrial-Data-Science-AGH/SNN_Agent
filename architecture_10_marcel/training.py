...
def extract_hpf_features(x: np.ndarray, sr: int, cfg: EncoderConfig) -> np.ndarray:
    """Per-frame [hf_ratio, zcr, crest] features used by the 10-neuron encoder."""
...
def extract_10_neuron_features(x: np.ndarray, sr: int, cfg: EncoderConfig) -> np.ndarray:
    """Map the audio features to seven L0 channels that mirror the documented 10-neuron architecture."""
    base = extract_hpf_features(x, sr, cfg)
    hf_ratio = np.clip(base[:, 0], 0.0, 1.0)
    zcr = np.clip(base[:, 1], 0.0, 1.0)
    crest = np.clip(base[:, 2], 0.0, 1.0)

    peak_fast = np.clip(0.6 * hf_ratio + 0.4 * crest, 0.0, 1.0)
    peak_strict = np.clip(np.maximum(hf_ratio - 0.2, 0.0) + 0.5 * crest, 0.0, 1.0)
    crest_factor = np.clip(crest, 0.0, 1.0)
    peak_counting = np.clip(np.minimum(zcr + hf_ratio, 1.0), 0.0, 1.0)
    cv = np.clip(np.minimum(0.5 * zcr + 0.5 * crest, 1.0), 0.0, 1.0)
    zcr_hf_proxy = np.clip(np.minimum(0.6 * zcr + 0.4 * hf_ratio, 1.0), 0.0, 1.0)
    continuity = np.clip(np.minimum(hf_ratio + 0.5 * zcr + 0.2 * crest, 1.0), 0.0, 1.0)

    return np.stack(
        [peak_fast, peak_strict, crest_factor, peak_counting, cv, zcr_hf_proxy, continuity],
        axis=1,
    ).astype(np.float32)


def encode_spikes(x: np.ndarray, sr: int, cfg: EncoderConfig) -> np.ndarray:
    feats = extract_10_neuron_features(x, sr, cfg)
    n_frames = feats.shape[0]
    frame_steps = max(1, int(round(cfg.frame_window_ms / cfg.dt_ms)))
    n_steps = n_frames * frame_steps
    n_channels = feats.shape[1]
    spikes = np.zeros((n_steps, n_channels), dtype=np.float32)

    smoothed = np.zeros(n_channels, dtype=np.float32)
    norm_vals = np.zeros((n_frames, n_channels), dtype=np.float32)
    max_vals = np.ones(n_channels, dtype=np.float32)
    ...
    last_spike_step = np.full(n_channels, -1_000_000, dtype=np.int64)
    curr_isi_steps = np.zeros(n_channels, dtype=np.int64)
    ttfs_armed = np.zeros(n_channels, dtype=bool)
    ttfs_time_step = np.zeros(n_channels, dtype=np.int64)
...
class TenNeuronGlassBreakSNN(nn.Module):
    """A 10-neuron-inspired SNN with 7 L0 detectors, 2 L1 combiners, and a decision neuron."""
    ...
    def __init__(self, n_hidden: int = 10, beta: float = 0.95) -> None:
        ...
        self.l0_lifs = nn.ModuleList(
            [snn.Leaky(beta=beta, threshold=0.5, learn_beta=True, learn_threshold=True) for _ in range(self.n_l0)]
        )
        self.lif_impact = snn.Leaky(beta=beta, threshold=0.5, learn_beta=True, learn_threshold=True)
        self.lif_fragmentation = snn.Leaky(beta=beta, threshold=0.5, learn_beta=True, learn_threshold=True)
        self.lif_decision = snn.Leaky(beta=beta, threshold=0.5, learn_beta=True, learn_threshold=True)

        self.impact_head = nn.Linear(3, 1)
        self.fragmentation_head = nn.Linear(3, 1)
        self.decision_head = nn.Linear(3, 1)

    def clamp_dynamics(self) -> None:
        """Keep learned beta/threshold values in a physically valid range."""
        with torch.no_grad():
            for lif in [*self.l0_lifs, self.lif_impact, self.lif_fragmentation, self.lif_decision]:
    ...
    def forward(self, spike_input: torch.Tensor) -> Tuple[torch.Tensor, dict]:
        if spike_input.dim() == 2:
            ...
            if spike_input.shape[1] == self.n_input:
                ...
                raise ValueError("Expected input with 7 channels in dim 1 or dim 2")
        else:
        ...
        l0_mem = [lif.init_leaky() for lif in self.l0_lifs]
        impact_mem = self.lif_impact.init_leaky()
        fragmentation_mem = self.lif_fragmentation.init_leaky()
        decision_mem = self.lif_decision.init_leaky()

        l0_rec = torch.zeros((n_timesteps, batch_size, self.n_l0), device=x.device)
        impact_rec = torch.zeros((n_timesteps, batch_size, 1), device=x.device)
        fragmentation_rec = torch.zeros((n_timesteps, batch_size, 1), device=x.device)
        decision_rec = torch.zeros((n_timesteps, batch_size, 1), device=x.device)
        mem_output_rec = torch.zeros((n_timesteps, batch_size, 1), device=x.device)

        for t in range(n_timesteps):
            cur_input = x[:, :, t]
            l0_step = []
            for i, lif in enumerate(self.l0_lifs):
                spk, l0_mem[i] = lif(cur_input[:, i].unsqueeze(1), l0_mem[i])
                l0_step.append(spk.squeeze(1))

            l0_tensor = torch.stack(l0_step, dim=1)
            l0_rec[t] = l0_tensor

            impact_in = l0_tensor[:, :3]
            fragmentation_in = l0_tensor[:, 3:6]
            continuity_in = l0_tensor[:, 6:7]

            impact_logits = self.impact_head(impact_in).squeeze(-1)
            fragmentation_logits = self.fragmentation_head(fragmentation_in).squeeze(-1)
            impact_spk, impact_mem = self.lif_impact(impact_logits, impact_mem)
            fragmentation_spk, fragmentation_mem = self.lif_fragmentation(fragmentation_logits, fragmentation_mem)

            decision_in = torch.cat(
                [impact_spk.unsqueeze(1), fragmentation_spk.unsqueeze(1), -continuity_in],
                dim=1,
            )
            decision_logits = self.decision_head(decision_in).squeeze(-1)
            decision_spk, decision_mem = self.lif_decision(decision_logits, decision_mem)

            impact_rec[t] = impact_spk.unsqueeze(1)
            fragmentation_rec[t] = fragmentation_spk.unsqueeze(1)
            decision_rec[t] = decision_spk.unsqueeze(1)
            mem_output_rec[t] = decision_mem.unsqueeze(1)

        logits = mem_output_rec.mean(dim=0)
        spike_rate = decision_rec.mean(dim=0)

        out = {
            "input": l0_rec.permute(1, 2, 0),
            "impact": impact_rec.permute(1, 2, 0),
            "fragmentation": fragmentation_rec.permute(1, 2, 0),
            "decision": decision_rec.permute(1, 2, 0),
...
def evaluate(model: TenNeuronGlassBreakSNN, loader: DataLoader, device: torch.device) -> dict:
    model.eval()
    total = 0
    correct = 0
    loss_sum = 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train a 3-N-1 LIF SNN for the 10-neuron glass-break architecture")
...
def main() -> None:
    ...
    if len(dataset) == 0:
        if args.manifest:
            raise SystemExit(f"No samples loaded from manifest: {args.manifest}")
    ...
    model = TenNeuronGlassBreakSNN(n_hidden=args.hidden_size)
    train(model, train_loader, test_loader, device, epochs=args.epochs, lr=args.lr, grad_clip=args.grad_clip)

    save_path = os.path.join(output_dir, "glassbreak_snn_model_10_neurons.pt")
    torch.save(model.state_dict(), save_path)
    print(f"Saved model state dict to {save_path}")

    print("\n=== Hardware Parameter Mapping ===")
    first_lif = model.l0_lifs[0]
    beta_val = first_lif.beta.item()
    threshold_val = first_lif.threshold.item()
    ...
    print(f"L0 layer: beta={beta_val_clamped:.4f}, tau_mem={tau_mem:.4f}, "
