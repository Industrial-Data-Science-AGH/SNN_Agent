...
def test_parse_good_json() -> None:
    raw = '{"is_intrusion": true, "confidence": 0.9, "reason": "person at window"}'
...
def test_parse_rejects_missing_field() -> None:
    with pytest.raises((ValueError, KeyError)):
        vision._parse_verdict('{"is_intrusion": true, "confidence": 0.9}')


def test_parse_strips_code_fence() -> None:
    raw = '```json\n{"is_intrusion": false, "confidence": 0.1, "reason": "cat"}\n```'
    v = vision._parse_verdict(raw)
...
def test_parse_clamps_confidence() -> None:
    raw = '{"is_intrusion": true, "confidence": 1.5, "reason": "overflow"}'
    v = vision._parse_verdict(raw)
...
def test_verdict_happy_path_source_gemini() -> None:
    v = vision.verdict(
        _SNAPSHOT,
        generate=lambda _s: '{"is_intrusion": true, "confidence": 0.9, "reason": "person at window"}',
    )
    assert v.is_intrusion is True
    assert v.source == "gemini"
...
def test_verdict_false_pass_through() -> None:
    v = vision.verdict(
        _SNAPSHOT,
        generate=lambda _s: '{"is_intrusion": false, "confidence": 0.1, "reason": "cat"}',
    )
    assert v.is_intrusion is False
    assert v.source == "gemini"
