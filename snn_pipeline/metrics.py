...
def thermal_resistance_score(
    ...
    """Oblicza odporność termiczną na podstawie spadku recall.

    Args:
        nominal_recall: Recall w warunkach nominalnych (25°C).
        worst_case_recall: Recall dla najgorszych warunków termicznych.
        thermal_drift_pct: Możliwa zmiana temperatury w procentach.

    Returns:
        Odporność termiczna jako float [0, 1], gdzie 1.0 oznacza brak degradacji.
    
    Przykład:
        >>> thermal = thermal_resistance_score(nominal_recall=0.92, worst_case_recall=0.87)
        >>> print(f"Thermal Resistance Score: {thermal:.2f}")
    """
    if nominal_recall == 0:
        return 0.0 # Nie dzielimy przez zero, recall = 0 -> zwracamy 0.0

    recall_change = worst_case_recall - nominal_recall
    recall_change_pct = abs(recall_change / nominal_recall)

    # Mapujamy recall_change_pct : 0% change → 1.0, 50% change → 0.5, 100% change → 0.0
