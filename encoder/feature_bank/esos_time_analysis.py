...
def analyze_mcu_time_consumption():
    ...
    print(
        f"Baza (RMS, DC, zcr_pre): {cycles_to_us(base_cycles, F_CPU):.2f} us ({base_cycles} cykli)"
    )

    fft_comp = get_shared_fft_complexity(HOP_SAMPLES, N_FFT_BINS)
    fft_cycles = fft_comp.cycles_cortex_m4f()
    print(
        f"Baza FFT (Okno + Trans.): {cycles_to_us(fft_cycles, F_CPU):.2f} us ({fft_cycles} cykli)"
    )
    print("-" * 65)
    ...
    print("-" * 65)
    print(
        f"CAŁKOWITY CZAS RAMKI: {cycles_to_us(total_cycles, F_CPU):.2f} us / {10000.0:.2f} us budżetu (10ms)"
    )
