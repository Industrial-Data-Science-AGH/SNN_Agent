...
# ------------------------------
#
# Input channels are now (hf_ratio, zcr, crest) instead of (peak, mean, std).
# These replace the original amplitude-envelope statistics, which turned out
# to have almost no class-separating power (Cohen's |d| ~ 0.05-0.09). The new
# features target frequency content and burst shape instead of raw loudness:
#
#   hf_ratio - energy of a narrow, high-cutoff filtered signal divided by
#              energy of a broader high-pass filtered signal. Approximates
#              "how much of this sound's energy sits at high frequency."
#   zcr      - zero-crossing rate of the broadband filtered signal per frame.
#              A cheap, filter-free proxy for dominant frequency content.
#   crest    - peak-to-RMS ratio of the broadband filtered signal per frame.
#              Captures impulsive/bursty shape (glass tinkling) vs. smoother
#              or more continuous sounds.
...
class GlassBreakSNN(nn.Module):
    ...
    def __init__(self, n_hidden: int = 10, beta: float = 0.95) -> None:
        ...
        # Signed, fan-in scaled initialization: allows inhibitory
        # connections and decorrelates hidden units from the start so they
        # don't all receive near-identical gradients.
    ...
    def forward(self, spike_input: torch.Tensor) -> Tuple[torch.Tensor, dict]:
        ...
        # Continuous logit used for the loss: time-averaged membrane
        # potential of the output neuron (unbounded, differentiable
        # everywhere, never saturates at a hard 0).
        logits = mem_output_rec.mean(dim=0)  # [batch, n_output]
        # Spike-rate trigger kept around for reporting / hardware parity.
...
def report_feature_separability(pooled_features: Sequence[Tuple[np.ndarray, int]]) -> None:
    ...
    crest) features actually differ between classes? If they barely differ,
    the SNN (or any classifier) has little to work with and the encoder /
    segment-selection stage needs attention before touching the model."""
    ...
    for i, name in enumerate(names):
        ...
        # Cohen's d as a rough effect-size indicator.
    ...
    print("(|d| ~0.2 small, ~0.5 medium, ~0.8+ large effect size; low values across "
          "the board mean the model has little signal to learn from.)\n")
...
def main() -> None:
    ...
    class_weights = 1.0 / torch.from_numpy(np.sqrt(np.maximum(class_counts, 1))).float()
