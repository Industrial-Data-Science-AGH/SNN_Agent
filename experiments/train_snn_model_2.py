...
def report_feature_separability(pooled_features: Sequence[Tuple[np.ndarray, int]]) -> None:
    """Cheap, dependency-free sanity check: do the pooled (peak, mean, std)
    features actually differ between classes? If they barely differ, the
    SNN (or any classifier) has little to work with and the encoder /
    ...
    names = ["peak", "mean", "std"]
    print("\n=== Feature separability check (pooled peak/mean/std) ===")
    for i, name in enumerate(names):
        ...
        print(f"  {name:5s}: pos={pm:8.2f}±{ps:6.2f}  neg={nm:8.2f}±{ns:6.2f}  |d|={d:5.2f}{flag}")
