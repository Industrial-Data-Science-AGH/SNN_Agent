...
def windowed_counts(train, w):
    ...
    c = np.cumsum(
        np.vstack([np.zeros((1, train.shape[1]), dtype=np.int32), train]), axis=0
    )
...
def main():
    ...
    print(
        f"[dane] {len(clips)} klipów ({int((labels == 1).sum())} glass), "
        f"tło {neg_hours:.2f} h",
        flush=True,
    )
    ...
    for i in range(3):
        print(
            f"[G{i}] średnia częstość: glass {rates[labels == 1, i].mean():.2f} Hz, "
    ...
    for w in args.windows:
        ...
        for a in itertools.product((-2, -1, 0, 1, 2), repeat=3):
            if all(x <= 0 for x in a):
                continue
            combo_max = np.array(
                [(c @ np.array(a)).max() if len(c) else -99 for c in counts]
            )
            for th in (1, 2, 3, 4):
                det = combo_max >= th
    ...
    for rec, fa, fa_q, a, th, w in pareto:
        if rec < 0.30:
            continue
        print(
            f"  {str(a):>16} >= {th}, {w * 10:4d}ms {100 * rec:6.1f}% {100 * fa:6.1f}% "
            f"{100 * fa_q:8.1f}%"
        )
        shown += 1
        if shown >= args.top:
