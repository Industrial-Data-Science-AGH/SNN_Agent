...
def collect_esc50(repo_root: Path) -> List[Entry]:
    ...
    for wav in sorted(audio_dir.glob("*.wav")):
        ...
        group_id = (
            f"esc50_{parts[0]}_{parts[1]}" if len(parts) >= 4 else f"esc50_{wav.stem}"
        )
        entries.append(
            Entry(
...
def collect_notebooks(repo_root: Path) -> List[Entry]:
    ...
    if entries and n_groups == len(entries):
        print(
            "[WARN] notebooks: każdy plik to osobna grupa — jeśli te nagrania "
...
def main() -> None:
    ...
    split_map = group_based_split(entries, args.val_frac, args.test_frac, RANDOM_SEED)
