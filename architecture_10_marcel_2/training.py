...
def extract_hpf_features(x: np.ndarray, sr: int, cfg: EncoderConfig) -> np.ndarray:
    """Per-frame [hf_ratio, zcr, crest] features -- the 3 input channels."""
...
class GlassBreakSNN(nn.Module):
    ...
    Input: 3 channels (hf_ratio, zcr, crest).
    Hidden: n_hidden fully-connected LIF neurons.
    Output: 1 decision neuron. Its membrane potential, averaged over time,
    is used as the logit (continuous, differentiable, never saturates at a
    hard 0 the way a binary spike-rate average can).
    """
    ...
    def clamp_dynamics(self) -> None:
        ...
        can blow up over long unrolls) or threshold <= 0 (degenerate, fires
        every step). Call this after every optimizer.step()."""
        with torch.no_grad():
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y in train_loader:
            ...
            # to physically invalid values (beta >= 1) mid-training.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a 3-N-1 LIF SNN for glass-break detection"
    )
    parser.add_argument(
        "--positive-dir",
...
def main() -> None:
    ...
    # Inverse-FREQUENCY (not inverse sqrt) weighting -- makes the expected
    # class fraction in every sampled batch exactly 50/50 regardless of how
    # skewed the raw counts are.
